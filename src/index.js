...
function isAllTrue(array, fn) {
    // Надо ли оборачивать в try/catch?
    if (array === undefined || array.length === 0 || !(array instanceof Array)) {
...
function isSomeTrue(array, fn) {
    ...
    let elementTrue = true;

    for (let i = 0; i < array.length; i++) {
        elementTrue = fn(array[i]);
        if (elementTrue) {
            return elementTrue;
        }
    }

    return false; // почему тест не пропускает возвращение !elementTrue?
}
